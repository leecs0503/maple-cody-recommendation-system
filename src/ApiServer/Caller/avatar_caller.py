...
class AvatarCaller:
    ...
    async def get_character_look_data(self, encrypted_character_look: str):
        url = f"{self.server_protocol}://{self.server_host}:{self.server_port}/character_look_data"
        retry_num = self.retry_num if self.retry_num >= 0 else 1000000000
        json_data = {"packed_character_look": encrypted_character_look}
        ...
        raise Exception("err: get_character_look_data")

    async def get_avatar_image(self, avatar_dict: Dict[str, str]):
        url = f"{self.server_protocol}://{self.server_host}:{self.server_port}/avatar_image"
        retry_num = self.retry_num if self.retry_num >= 0 else 1000000000
        json_data = {"avatar": avatar_dict}
        ...
        raise Exception("err: get_avatar_image")
